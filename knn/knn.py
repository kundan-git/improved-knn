...
class KNN():
     ...
     def _build_k_filed_density_dict(self):    
         ...
         #print self.k_filed_density_dict
     ...
     def _get_decison(self, nn_tuple):
         ...
         if self.decision_type == self.decision_function_types[0]:
             ...
             return self._get_sim_and_density_wtd_majority_voting_descison(nn_tuple)
     ...
     def _get_sim_and_density_wtd_majority_voting_descison(self,nn_tuple):
         #TODO
         cat_to_vote = {}
         for dist_cat_idx in nn_tuple:
             ...
             #print sim,  cat,idx_in_train_data
             if not cat_to_vote.__contains__(cat):
                 cat_to_vote[cat] = 1*sim*self.k_filed_density_dict[idx_in_train_data]
             else:
                 cat_to_vote[cat] += 1*sim*self.k_filed_density_dict[idx_in_train_data]
         sorted_tuple_list = sorted(cat_to_vote.items(),key=lambda x:x[1], reverse=True)
         return sorted_tuple_list[0][0]
...
if __name__=="__main__":
    ...
    data_set_ranges = [100,200,300,400,500,600,700,800,900,1000,1250,1500]
    knn_k_ranges = [5,10,15,20,25]
    final_eval = {}
        ...
        for knr in knn_k_ranges:
            ...
            for test_train_tup in test_train_tup_list:
                ...
                for item in evaluation.items():
                    print item[0],item[1]
                print "Completed a fold for dataset "+ str(dsr)+"...\n"
